...
void BuildMaxHeap(ElemTtype* A,int len){
	for(int i = len/2-1; i>=0; --i)		//从最后一个非叶节点到根节点，反复调整堆（节点从0~len-1编号）
		AdjustDown(A,i,len);
}
void AdjustDown(ElemTtype* A,int k, int len){
	//if( k > len/2-1 || len==1)	// len/2-1最后一个非叶节点（比较绕，直接改用判断其 左孩子节点 是否存在，从而判断是否为叶子节点）
	if( 2*k+1 >= len)		//!!!!-叶子节点(判断条件：如果有左孩子节点，左孩子节点是否大于最后一个节点下标)，不用调整，直接返回---递归结束条件
		return;
	//找到当前非叶节点的最大的孩子节点
