...
int main(){
	...
	ListNode *head = NULL;
	// 测试：
	RemoveNode(&head,2); //测试：链表为空时删除
	printListZF(head);  // 正向反向打印

	AddToTail(&head,1);	//测试：添加第一个节点；
	//test.AddToTail_useReference(head,1);	//测试：添加第一个节点；
	printListZF(head);  // 正向反向打印

	RemoveNode(&head,1);	//测试：只有一个节点时删除；
	printListZF(head);  // 正向反向打印

    AddToTail(&head,1);	//测试：删除后添加第一个节点；
	printListZF(head);  // 正向反向打印
	
	AddToTail(&head,2);	//测试：添加；
	printListZF(head);  // 正向反向打印

	AddToTail(&head,3);	//测试：添加；
	printListZF(head);  // 正向反向打印

	AddToTail(&head,4);	//测试：添加；
	printListZF(head);  // 正向反向打印

	AddToTail(&head,5);	//测试：添加；
	printListZF(head);  // 正向反向打印

	AddToTail(&head,6);	//测试：添加；
	printListZF(head);  // 正向反向打印
	
	RemoveNode(&head,1);	//测试：删除开头
	printListZF(head);  // 正向反向打印

	RemoveNode(&head,4);	//测试：删除中间
	printListZF(head);  // 正向反向打印

	RemoveNode(&head,6);	//测试：删除最后
	printListZF(head);  // 正向反向打印
